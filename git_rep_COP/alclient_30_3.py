...
def execute(s, tc):
	...
	for i in range(len(lis)):
		...
		data=s.recv()
		if not data: break
		if len(lis[3:-3]) != len(list(data)):
			eroo = raw_input("Test case '" + tc + "' failed: Enter 'c' to continue or 's' to stop the test:")
			if eroo == "c":
				continue
			elif eroo == "s":
				break
		elif eroo == "":
			for i in range(len(lis[3:-3])):
				if not lis[3:-3][i] in list(data):			
					eroo = "1"
			if eroo == "1":
				eroo = raw_input("Test case '" + tc + "' failed: Enter 'c' to continue or 'e' to stop the test:")
				if eroo == "c":
					continue
				elif eroo == "s":
					break
...
if wut == 'r':
	...
	wq=raw_input("Please enter the name of test case to be executed:")
	execute(s,wq)
