...
def Main():
	...
	global l
	l=0
	cmd='sudo sh -c "tail -0f /opt/arbd/logs/arbd.log_2018-02-21_17-46 > reco.txt"'
	...
	while l==0:
		...
		# command will be of the form "['K1','K2',..]"   , command is a string
 	    if not msg:
			...
			msg=str(msg)
			lst,delta = msg.split("::")[0],float(msg.split("::")[1])

			try :
				exec("command_list="+lst)  #command_list is a list of the form ['k=uinput.KEY_K1','k=uinput.KEY_K2',...]
				combo=[]
